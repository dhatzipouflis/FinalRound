...
using FinalRound.Identity.Api.Extensions;
...
builder.Services.Configure<ApiBehaviorOptions>(opt =>
{
    ...
    {
        var correlationId = ctx.HttpContext.Request.Headers.TryGetValue(
            CorrelationTraceMiddleware.CorrelationHeader, out var v)
            ? v.ToString()
        ...
        var errors = ctx.ModelState
            ...
            .SelectMany(x => x.Value!.Errors.Select(e =>
                new ApiError(
                    "validation_error",
                    string.IsNullOrWhiteSpace(e.ErrorMessage) ? "Validation error" : e.ErrorMessage)))
            .ToArray();

        return new OkObjectResult(
            ApiResponse<object>.Fail(errors, ctx.HttpContext.TraceIdentifier, correlationId));
    };
});
